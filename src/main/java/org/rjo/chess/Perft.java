...
public class Perft {
	...
	public static void main(String[] args) {
		...
		// (updateStructures 100% incremental)
	...
	public static List<String> findMovesDebug(Game game, Colour sideToMove, int depth) throws IOException {
		...
		try (Writer debugWriter = new BufferedWriter(new FileWriter(file))) {
			return findMovesInternalDebug(game.getPosition(), sideToMove, depth, new ArrayDeque<String>(1000),
					new ArrayList<String>(500000), debugWriter);
		}
	}
	...
	private static List<String> findMovesInternalDebug(Position posn, Colour sideToMove, int depth,
		...
		for (Move move : posn.findMoves(sideToMove)) {
			...
			List<String> movesFromThisPosn = findMovesInternalDebug(posnAfterMove, Colour.oppositeColour(sideToMove),
					depth - 1, movesSoFar, totalMoves, debugWriter);
