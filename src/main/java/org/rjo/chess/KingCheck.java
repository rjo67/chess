...
public class KingCheck {
	 ...
	 * <p>
	 * Examines opponent's knights and pawns for checks, and all rays. This is therefore suitable for a game situation where the king has just moved,
	 * or the king was in check beforehand.
	 * <p>
	 * For an optimized version use {@link #isKingInCheck(Square, Colour, BitSet, Map, RayType)}, specifying the 5th parameter.
	 *
	 ...
	 * Returns true if the king is currently in check. NB stops as soon as at least one check is discovered.
	 * <p>
	 * Suitable for multithreaded use, since does not modify any global state. TODO: could be expanded to return which piece is checking.
	 *
	 * @param kingsSquare where the king is.
	 * @param kingsColour colour of the king.
	 * @param friendlyPieces bitset indicating location of the friendly pieces.
	 * @param enemyPieces bitsets indicating location of the enemy pieces.
	 * @param rayToExamine if set, JUST this ray will be examined. This is an optimization where the king was not in check beforehand. Then we only
	 *           need to check the ray which has been vacated by the moving piece. <b>Do not set</b> if the king himself has moved.
	 * @return true if the king is in check.
	 */
	public static boolean isKingInCheck(Square kingsSquare, Colour kingsColour, BitSet friendlyPieces,
			BitSet[] enemyPieces, RayType rayToExamine) {

		boolean optimizedRaySearch = (rayToExamine != null);

		/*
		 * The algorithm first handles the special cases of pawn or knight checks. Then, for each ray emenating from the king's square, the squares on
		 * the ray get checked. If the square contains an enemy piece then this is checked for a possible check.
		 */
		...
		if (!optimizedRaySearch) {
			...
			if (Pawn.attacksSquare(kingsSquare, Colour.oppositeColour(kingsColour),
		...
		for (RayType rayType : raysToCheck) {
			...
			while (rayIter.hasNext()) {
				...
				if (allEnemyPieces.getBitSet().get(bitIndex)) {
					...
					if (ray.isDiagonal()
							&& (queenPresentAtBitIndex || enemyPieces[PieceType.BISHOP.ordinal()].get(bitIndex))) {
						return true;
					} else if (!ray.isDiagonal()
							&& (queenPresentAtBitIndex || enemyPieces[PieceType.ROOK.ordinal()].get(bitIndex))) {
						return true;
					} else {
						break;
					}
	 ...
	 * <p>
	 * Helper-Method, delegates to {@link #isKingInCheckAfterMove(Square, Colour, BitSet, Map, Move, boolean)} with last parameter==true.
	 * <p>
	 * <b>Use this procedure if the king was already in check before the given move.</b>
	 *
	 * @param kingsSquare where the king is. If 'move' indicated that the king has moved, this value will be ignored and the king's new square will be
	 *           calculated.
	 * @param kingsColour colour of the king.
	...
	public static boolean isKingInCheckAfterMove_PreviouslyWasInCheck(Square kingsSquare, Colour kingsColour,
			BitSet friendlyPieces, BitSet[] enemyPieces, Move move) {
	 ...
	 * Returns true if the king would be in check after the given move.
	 * <p>
	 * Helper-Method, delegates to {@link #isKingInCheckAfterMove(Square, Colour, BitSet, Map, Move, boolean)} with last parameter==false.
	 * <p>
	...
	/**
	 * Returns true if the king would be in check after the given move. This method can be called directly, but there are helper methods available (see
	 * isKingInCheckAfterMove_PreviouslyNotInCheck and isKingInCheckAfterMove_PreviouslyWasInCheck).
	 * <p>
	 * <b>If the king is already in check before the given move, you must set the parameter kingWasInCheck to true.</b>
	 * <p>
	 * An optimised search is used. Assuming the king himself has not moved, and the king was not previously in check, only the ray given by the king's
	 * square and the vacated square needs to be examined.
	 *
	 * @param kingsSquare where the king is. If 'move' indicated that the king has moved, this value will be ignored and the new king's square will be
	 *           calculated.
	 * @param kingsColour colour of the king.
	 * @param friendlyPieces bitset indicating location of the friendly pieces (pre-move).
	 * @param enemyPieces bitsets indicating location of the enemy pieces (pre-move).
	 * @param move the move to make
	 * @param kingWasInCheck indicates that the king was in check before this move. Therefore, cannot use the optimized ray search.
	 * @return true if the king would be in check after the move.
	 */
	public static boolean isKingInCheckAfterMove(Square kingsSquare, Colour kingsColour, BitSet friendlyPieces,
			BitSet[] enemyPieces, Move move, boolean kingWasInCheck) {
