...
public class Rook extends SlidingPiece {
   ...
   /*
    * if useMovemap is defined, we'll use the data structures moveMap, vertMoveMap.
    * Otherwise the ray algorithm will be used.
    */
...
// @formatter:off
   ...
   static {
      ...
      /*
       * general algorithm:
       * 'tmpMoveMap' stores the bitmaps to the 'left' and the 'right' of piecePosn.
       * In the 3rd for loop, these get concatenated into 'concatMoveMap' -- one value for both the LHS and the RHS.
       * These values are then stored as immutable maps.
       *
   ...
   private List<Move> findMovesUsingMoveMap(Game game) {
    ...
    *           bit index of our piece for which the moves are currently being calculated
   ...
   public List<Move> findMoves(Game game, boolean kingInCheck) {
      ...
      // search for moves
      if (USE_MOVE_MAP) {
         moves = findMovesUsingMoveMap(game);
      } else {
