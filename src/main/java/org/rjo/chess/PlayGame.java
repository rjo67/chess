...
public class PlayGame {
   ...
   private void checkValidity(Move move, Game game) throws IllegalArgumentException {
      ...
      if (move.getPiece() != game.getChessboard().pieceAt(move.from(), game.getSideToMove())) {
      ...
      if (!move.isCapture()) {
         ...
         if (!game.getChessboard().getAllPieces(Colour.oppositeColour(game.getSideToMove())).getBitSet()
   ...
   private Move getMove(Game game, BufferedReader in) throws IllegalArgumentException, IOException {
      ...
      if ("O-O".equals(moveStr)) {
         m = Move.castleKingsSide(game.getSideToMove());
      } else if ("O-O-O".equals(moveStr)) {
         m = Move.castleQueensSide(game.getSideToMove());
      } else {
         ...
         if (!((moveStr.charAt(startOfFromSquare + 2) == 'x') || (moveStr.charAt(startOfFromSquare + 2) == '-'))) {
            throw new IllegalArgumentException("invalid input. Expected 'x' or '-' at position "
                  + (startOfFromSquare + 3));
         }
