...
public class Perft {
   ...
   public static void main(String[] args) {
      ...
      var nbrIters = 1;
      LOG_MOVES = false;
      if (args.length >= 1) { nbrThreads = Integer.parseInt(args[0]); }
      if (args.length >= 2) { startDelay = Integer.parseInt(args[1]); }
      if (args.length >= 3) { nbrIters = Integer.parseInt(args[2]); }
      if (startDelay > 0) {
         System.out.println(String.format("Starting in %s ms ...", startDelay));
         try {
      ...
      long totalTime = 0;
      int totalMoves = 0;
      for (int iter = 0; iter < nbrIters; iter++) {
         System.out.println(
               String.format("Iteration %2d. %s::posn6ply%d starting (%d threads)...", (iter + 1), Perft.class.getCanonicalName(), REQD_DEPTH, nbrThreads));
         ...
         totalTime += time;
         totalMoves += moves;
         System.out.println(String.format(Locale.GERMANY, "%dply: %,12d moves (%,6d ms) (%7.1f moves/ms) (%3.1f nanos/move)", REQD_DEPTH, moves, time,
               ((moves * 1.0) / time), ((sw.getNanoTime() * 1.0) / moves)));
         if (moves != EXPECTED_MOVES[REQD_DEPTH - 1]) { System.out.println("ERROR: wrong number of moves"); }
         System.gc();
      }
      if (nbrIters > 1) {
         System.out.println(String.format(Locale.GERMANY, "average of %d iterations: %5.2f ms (%7.1f moves/ms)", nbrIters, (totalTime * 1.0) / nbrIters,
               ((totalMoves * 1.0) / totalTime)));
      }
   }
    ...
    */
   public static int findAndCountMoves(Position posn, Colour sideToMove, int depth, int nbrThreads) {
    ...
    * @param sideToMove the starting colour
    * @param depth      the required depth to search
    * @return a map containing all the start moves for the <code>sideToMove</code> and for each map entry, a number representing how many leaf
    *         nodes there are from this starting move.
    */
   public static Map<String, Integer> findMoves(Position posn, Colour sideToMove, int depth, int nbrThreads) {
      if (nbrThreads == 1) {
   ...
   public static Map<String, Integer> findMovesSingleThreaded(Position posn, Colour sideToMove, int depth) {
   ...
   public static Map<String, Integer> findMovesMultiThreaded(Position posn, Colour sideToMove, int depth, int nbrThreads) {
      if (depth < 1) { throw new IllegalArgumentException("depth must be >= 1"); }
      ...
      try {
         ...
         if (!threadPool.isTerminated()) { System.err.println("cancel non-finished tasks"); }
   ...
   /**
    * Like {@link #findMoves(Position, Colour, int, int)} but with the option of storing the moves in a file. (Now uses MOVE_LOGGER at level
    * TRACE.) Uses more memory than the other method and is therefore not recommended. <b>This is mainly for PerftTests</b>. We return List of
    * String instead of List of Move to try to conserve memory.
    *
    * @param game       the game
    * @param sideToMove the starting colour
    * @param depth      the required depth to search
    ...
    */
   public static List<String> findMovesDebug(Game game, Colour sideToMove, int depth) {
    ...
    * @return the list of possible moves at the given depth. The string representation is stored in order to save memory.
    */
   private static List<String> findMovesInternalDebug(Position posn, Colour sideToMove, int depth, Deque<String> movesSoFar, List<String> totalMoves) {
      ...
      for (IMove move : posn.findMoves(sideToMove)) {
         ...
         List<String> movesFromThisPosn = findMovesInternalDebug(posnAfterMove, sideToMove.opposite(), depth - 1, movesSoFar, totalMoves);
         if (movesFromThisPosn.isEmpty()) {
            totalMoves.add(move.toString());
            if (MOVE_LOGGER.isTraceEnabled()) { movesAtThisLevel.add(move.toString()); }
         }
         int size = totalMoves.size();
         if (size % 500000 == 0) { System.out.println(size); }

         if (MOVE_LOGGER.isTraceEnabled()) { movesSoFar.removeLast(); }
      }

      if (MOVE_LOGGER.isTraceEnabled() && !movesAtThisLevel.isEmpty()) {
         boolean check = false;
         boolean capture = false;
         if (!movesSoFar.isEmpty()) { check = movesSoFar.peekLast().contains("+"); }
         if (!movesSoFar.isEmpty()) { capture = movesSoFar.peekLast().contains("x"); }
         MOVE_LOGGER.trace((check ? "CHECK" : "") + (capture ? "CAPTURE" : "") + " moves: " + movesSoFar + " -> " + movesAtThisLevel.size() + ":"
               + movesAtThisLevel + System.lineSeparator());
      }
      return totalMoves;
   }
}
