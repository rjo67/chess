...
public class UCI {
   ...
   private MoveInfo _findMove(SearchStrategy strategy) {
      ...
      if (lastMove.getPiece() == PieceType.PAWN) { game.getZobristMap().clear(); }
   ...
   private boolean inEndgame(Map<PieceType, Integer>[] pieceCounts) {
      ...
      for (Colour colour : Colour.ALL_COLOURS) {
         int points = (9 * pieceCounts[colour.ordinal()].get(PieceType.QUEEN)) + (5 * pieceCounts[colour.ordinal()].get(PieceType.ROOK))
               + (3 * pieceCounts[colour.ordinal()].get(PieceType.BISHOP)) + (3 * pieceCounts[colour.ordinal()].get(PieceType.KNIGHT))
      ...
      if ("moves".equals(subcmd)) {
         ...
         while (lineScanner.hasNext()) {
            ...
            if (lastmove) {
               ...
               boolean incheck = game.getPosition().squareIsAttacked(kingsSquare, Colour.oppositeColour(game.getPosition().getSideToMove()));
               m.setCheck(incheck);
