...
public class PlayGame {
   ...
   private void checkValidity(Move move, Game game) throws IllegalArgumentException {
      ...
      if (!move.isCapture()) {
         if (game.getPosition().getTotalPieces().get(move.to().bitIndex())) { throw new IllegalArgumentException("square " + move.to() + " not empty"); }
      } else {
         // if capture: opponent's piece at given square?
         if (!game.getPosition().getAllPieces(Colour.oppositeColour(game.getPosition().getSideToMove())).get(move.to().bitIndex())) {
   ...
   private Move getMove(Game game, BufferedReader in) throws IllegalArgumentException, IOException {
      ...
      if (moveStr.charAt(moveStr.length() - 1) == '+') { m.setCheck(true); }
